...
public class ClosedCases extends CasesProcessor {
	...
	protected void initializeTableSorter(IWContext iwc) throws RemoteException {
		...
		buffer.append("$(document).ready(function() { $('#" + getBlockID() + "').tablesorter( { headers: { " + (getBusiness().useTypes() ? 3 : 2) + ": { sorter: false }, " + (getBusiness().useTypes() ? 6 : 5) + ": { sorter: false}" + (showCheckBox() ? ", " + (getBusiness().useTypes() ? 7 : 6) + ": { sorter: false}" : "") + "}, sortList: [[0,1]] } ); } );");
